...
import { map, size, filter, rest } from "lodash";
...
export default function AddRestauranForm(props) {
  ...
  const addRestaurant = () => {
    if (!restaurantName || !restaurantAddress || !restaurantDescription) {
      toastRef.current.show("Todos los campos del formulario son obligatios.");
      ...
      uploadImageStorage().then((response) => {
        db.collection("restaurants")
          ...
          .catch(() => {
            setIsLoading(false);
            toastRef.current.show(
              "Error al subir el restaurante, inéntelo más tarde.",
            );
          });
      });
    }
